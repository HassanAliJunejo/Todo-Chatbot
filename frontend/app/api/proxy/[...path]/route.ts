...
async function handler(req: Request, ctx: { params: Promise<{ path: string[] }> }) {
  try {
    ...
    // Handle task endpoints - fix the path transformation
    else if (transformedPath.startsWith('/api/me/tasks')) {
      transformedPath = transformedPath.replace('/api/me/tasks', '/api/v1/me/tasks');
    }
    ...
    try {
      ...
      // For task endpoints, return empty array when backend is down
      if (transformedPath.includes('/tasks')) {
